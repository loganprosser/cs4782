...
The formula as a function of position i and embedding dimension j where all even dimensions are defined as:

\[
\text{PE}(i, j) = \begin{cases} \sin\!\left(i / 10000^{j/d_{\text{model}}}\right) & \text{if } j \text{ is even} \\ \cos\!\left(i / 10000^{(j-1)/d_{\text{model}}}\right) & \text{if } j \text{ is odd} \end{cases}
\] 

A benefit of this sinusoidal encoding is that it allows the model to generalize to input lengths longer than those that have been seen in training because the relative position between any two tokens can be writen through a linear combination of the exisiting encoding values. 
...
The most impressive performance increase is when we apply word2vect to logistic regression. I find it odd that LSTM and LR + W2V have such similar performance. distilBERT is the only truely pretrained model here and it provides significant better test accuracy over all other models at this task. Additional, W2V could be considered pretrained and made a massive improvement over the base logistic regression model.  
